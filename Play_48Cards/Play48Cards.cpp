...
IMPLEMENT_DYNAMIC(CPlay48Cards, CDialogEx)

CPlay48Cards::CPlay48Cards(CWnd* pParent /*=NULL*/)
	: CDialogEx(IDD_DIALOG_48CARDS, pParent)
	, m_nTimer(0)
	, m_strEditHint(_T(""))
	, m_str(_T(""))
	, inputNum(0)
...
CPlay48Cards::~CPlay48Cards()
{
}

void CPlay48Cards::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CPlay48Cards, CDialogEx)
	ON_WM_PAINT()
	ON_WM_TIMER()
	ON_WM_LBUTTONDOWN()
	ON_BN_CLICKED(IDC_BUTTON_HELP, &CPlay48Cards::OnClickedButtonHelp)
	ON_BN_CLICKED(IDC_BUTTON_HINT, &CPlay48Cards::OnClickedButtonHint)
	ON_BN_CLICKED(IDC_BUTTON_INPUT, &CPlay48Cards::OnClickedButtonInput)
	ON_WM_SYSCOMMAND()
	ON_WM_CTLCOLOR()
END_MESSAGE_MAP()


// CPlay48Cards 메시지 처리기입니다.
BOOL CPlay48Cards::OnInitDialog()
{
	...
	{
		CString str;
		MoveWindow(0, 0, 1700, 1020);
		m_prgsTimer.SetRange(0, 180);
		m_prgsTimer.SetPos(180);
		...
		{
			str.Format(L"..\\maplestory\\%03d.bmp", i);
...
void CPlay48Cards::OnPaint()
{
	...
	memDC.DeleteDC();
		...
		if (m_nCount == 0 && m_nCheck == FALSE) {
			...
			CString msg;
			KillTimer(2);
			m_nTimer = 180 - (m_prgsTimer.GetPos());
			msg.Format(_T("%d 점\n%d 초"), (180 - m_nTimer) * 180, m_nTimer);
...
void CPlay48Cards::OnTimer(UINT_PTR nIDEvent)
{
	...
	else if (nIDEvent == 2)
	{
		...
		{
			KillTimer(2);
			if (AfxMessageBox(_T("Game Over"), MB_ICONWARNING) == IDOK) {
				m_Flag = 3;
				m_nTimer = 180;
				EndDialog(IDOK);
			}
...
void CPlay48Cards::OnLButtonDown(UINT nFlags, CPoint point)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
	...
	if (point.x >= 0 && point.y >= 0 && point.x <=8 * 140 && point.y <= 6 * 160)
	{
		int x = point.x / 140;
		int y = point.y / 160;
...
void CPlay48Cards::OnClickedButtonHelp()
{
...
void CPlay48Cards::OnClickedButtonHint()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	SetTimer(1, 5000, NULL);
	m_Flag = 1;
...
void CPlay48Cards::OnClickedButtonInput()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	UpdateData(TRUE);
	CString strAns;
...
void CPlay48Cards::OnSysCommand(UINT nID, LPARAM lParam)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
	if (nID == SC_CLOSE) {
...
BOOL CPlay48Cards::PreTranslateMessage(MSG* pMsg)
{
...
HBRUSH CPlay48Cards::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
