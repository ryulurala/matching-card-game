...
IMPLEMENT_DYNAMIC(CPlay24Cards, CDialogEx)

CPlay24Cards::CPlay24Cards(CWnd* pParent /*=NULL*/)
	: CDialogEx(IDD_DIALOG_24CARDS, pParent)
	, m_nTimer(0)
	, m_strEditHint(_T(""))
	, m_str(_T(""))
	, inputNum(0)
...
CPlay24Cards::~CPlay24Cards()
{
}

void CPlay24Cards::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CPlay24Cards, CDialogEx)
	ON_WM_PAINT()
	ON_WM_LBUTTONDOWN()
	ON_WM_TIMER()
	ON_WM_SYSCOMMAND()
	ON_BN_CLICKED(IDC_BUTTON_HINT, &CPlay24Cards::OnClickedButtonHint)
	ON_BN_CLICKED(IDC_BUTTON_INPUT, &CPlay24Cards::OnClickedButtonInput)
	ON_BN_CLICKED(IDC_BUTTON_HELP, &CPlay24Cards::OnClickedButtonHelp)
	ON_WM_CTLCOLOR()
END_MESSAGE_MAP()


// CPlay24Cards 메시지 처리기입니다.


BOOL CPlay24Cards::OnInitDialog()
{
	...
	if (dialog == 1)
	{
		MoveWindow(0, 0, 1100, 800);
		CString str;
		m_prgsTimer.SetRange(0, 90);
		m_prgsTimer.SetPos(90);
		...
		for (int i = 0; i < MAX_CARD_COUNT; i++) {
			str.Format(L"..\\24cards\\%03d.bmp", i);
...
void CPlay24Cards::OnPaint()
{
	...
	dc.BitBlt(0, 0, 840, 640, &memDC, 0, 0, SRCCOPY);//
	...
	memDC.DeleteDC();
		...
		if (m_Flag == 1) {
			for (i = 0; i < REAL_CARD_COUNT * 2; i++) {
				if (m_CardTable[i] != -1) {
					...
					m_CardImage[index].Draw(dc, (i % 6) * 140, (i / 6) * 160);
				}
			}
		}
			...
			else
			{
				...
				for (i = 0; i < REAL_CARD_COUNT * 2; i++) {
					if (m_CardTable[i] != -1) 
					{
						m_CardImage[0].Draw(dc, (i % 6) * 140, (i / 6) * 160);
						m_nCount++;
					}
				}
			}
		...
		if (m_nCount == 0 && m_nCheck == FALSE) {
			...
			CString msg;
			KillTimer(2);
			m_nTimer = 90 - (m_prgsTimer.GetPos());
			msg.Format(_T("%d 점\n%d 초"), (180 - m_nTimer) * 90, m_nTimer);
...
void CPlay24Cards::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (point.x >= 0 && point.y >= 0 && point.x <= 6 * 140 && point.y <= 4 * 160) {
		int x = point.x / 140;
		int y = point.y / 160;
		int selectIndex = y * 6 + x;
...
void CPlay24Cards::OnTimer(UINT_PTR nIDEvent)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
	if (nIDEvent == 1) {
		KillTimer(1);
...
void CPlay24Cards::OnSysCommand(UINT nID, LPARAM lParam)
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
	if (nID == SC_CLOSE) {
...
void CPlay24Cards::OnClickedButtonHint()
{
...
void CPlay24Cards::OnClickedButtonInput()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	UpdateData(TRUE);
	CString strAns;
	...
	if (strAns.Left(3) == strHint.Left(3))
	{
...
void CPlay24Cards::OnClickedButtonHelp()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	CHelpDialog CHelpDialog;
	CHelpDialog.DoModal();
...
HBRUSH CPlay24Cards::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
BOOL CPlay24Cards::PreTranslateMessage(MSG* pMsg)
{
